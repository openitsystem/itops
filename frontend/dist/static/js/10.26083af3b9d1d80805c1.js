...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_vue_loader_13_7_3_vue_loader_lib_selector_type_script_index_0_groupexchangevalue_vue__ = __webpack_require__(2203);
/* empty harmony namespace reexport */
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__node_modules_vue_loader_13_7_3_vue_loader_lib_template_compiler_index_id_data_v_1ba00132_hasScoped_false_transformToRequire_video_src_poster_source_src_img_src_image_xlink_href_buble_transforms_node_modules_vue_loader_13_7_3_vue_loader_lib_selector_type_template_index_0_groupexchangevalue_vue__ = __webpack_require__(2234);
...
var Component = normalizeComponent(
  __WEBPACK_IMPORTED_MODULE_0__babel_loader_node_modules_vue_loader_13_7_3_vue_loader_lib_selector_type_script_index_0_groupexchangevalue_vue__["a" /* default */],
  __WEBPACK_IMPORTED_MODULE_1__node_modules_vue_loader_13_7_3_vue_loader_lib_template_compiler_index_id_data_v_1ba00132_hasScoped_false_transformToRequire_video_src_poster_source_src_img_src_image_xlink_href_buble_transforms_node_modules_vue_loader_13_7_3_vue_loader_lib_selector_type_template_index_0_groupexchangevalue_vue__["a" /* default */],
  ...
  methods: {
      ...
      this.$confirm('此操作将' + smtpvalue + '设置为主SMTP, 是否继续?', '提示', {
        ...
        beforeClose: function beforeClose(action, instance, done) {
          if (action === 'confirm') {
            ...
            .get(_this2.serviceurl() + '/api/SetDistrGroup/?CountName=' + disNameforurl + '&parametername=PrimarySmtpAddress&parametervalue=' + smtpvalue).then(function (response) {
              instance.confirmButtonLoading = false;
              if (response.data.isSuccess) {
                _this2.messagealertvalue('设置成功', 'success');
                _this2.getcomputermessagevalue();
              } else {
                if (response.data.message === '权限不足') {
                  _this2.messagealertvalue('权限不足', 'error');
                } else {
                  _this2.messagealertvalue('设置失败', 'error');
                }
              }
              done();
            });
    ...
    savechangeauthOrig: function savechangeauthOrig() {
      ...
      __WEBPACK_IMPORTED_MODULE_2_axios___default.a.get(this.serviceurl() + '/api/ChangeUserMessagebylist/?CountName=' + disNameforurl + '&Attributes=authOrig' + ChangeMessage).then(function (response) {
        if (response.data.isSuccess) {
          ...
          _this3.dialogauthOrig = false;
        } else {
          if (response.data.message === '权限不足') {
            _this3.messagealertvalue('权限不足', 'error');
          } else {
            _this3.messagealertvalue('发件权限修改失败', 'error');
          }
        }
      });
    },
      ...
      __WEBPACK_IMPORTED_MODULE_2_axios___default.a.get(this.serviceurl() + '/api/ChangeUserMessage/?CountName=' + this.sAMAccountName + '&Attributes=' + Attributesvalue + '&ChangeMessage=' + ChangeMessagevalue).then(function (response) {
        if (response.data.isSuccess) {
          ...
          if (Attributesvalue === 'msExchRequireAuthToSendTo') {
            _this4.msExchRequireAuthToSendTo = _this4.msExchRequireAuthToSendToChangevalue;
            _this4.changespanshow.changemsExchRequireAuthToSendToshow = true;
          }
        } else {
          if (response.data.message === '权限不足') {
    ...
    changeauthOrigdiagshow: function changeauthOrigdiagshow() {
      ...
      __WEBPACK_IMPORTED_MODULE_2_axios___default.a.get(this.serviceurl() + '/api/GetGroupPreMessage/?CountName=' + disNameforurl).then(function (response) {
        if (response.data.isSuccess) {
          _this5.dialogauthOrig = true;
          for (var i = 0; i < response.data.message.authOrig.length; i++) {
            _this5.authOriglist.push({ DN: response.data.message.authOrig[i] });
          }
          if (_this5.authOriglist.length === 0) {
            _this5.authOrigtypechangevalue = true;
            _this5.authOrigtype = true;
          } else {
            _this5.authOrigtypechangevalue = false;
            _this5.authOrigtype = false;
          }
        } else {
          if (response.data.message === '权限不足') {
            _this5.messagealertvalue('权限不足', 'error');
          } else {
            _this5.messagealertvalue('群组邮箱信息获取失败', 'error');
          }
        }
      });
    },
    changemailboxvalue: function changemailboxvalue(Attributesname, ChangeMessage) {
      ...
      var disNameforurl = this.getQueryVariable('disName');
      this.vLoadingShow = true;
      __WEBPACK_IMPORTED_MODULE_2_axios___default.a.get(this.serviceurl() + '/api/SetDistrGroup/?CountName=' + this.distinguishedName + '&parametername=' + Attributesname + '&parametervalue=' + ChangeMessage).then(function (response) {
        _this6.vLoadingShow = false;
        if (response.data.isSuccess) {
          if (Attributesname === 'RulesQuota') {
            _this6.RulesQuota = ChangeMessage;
            _this6.changespanshow.changeRulesQuotashow = true;
          } else if (Attributesname === 'RecipientLimits') {
            _this6.RecipientLimits = ChangeMessage;
            _this6.changespanshow.changeRecipientLimitsshow = true;
          } else if (Attributesname === 'Alias') {
            _this6.Alias = ChangeMessage;
            _this6.changespanshow.changeAliasshow = true;
            if (_this6.EmailAddressPolicyEnabled === 'True' || _this6.EmailAddressPolicyEnabled === 'true' || _this6.EmailAddressPolicyEnabled === true) {
              _this6.getcomputermessagevalue();
            }
          } else if (Attributesname === 'EmailAddressPolicyEnabled') {
            _this6.EmailAddressPolicyEnabled = ChangeMessage;
            _this6.changespanshow.changeEmailAddressPolicyEnabledshow = true;
            // if ()
            _this6.getcomputermessagevalue();
          } else if (Attributesname === 'PrimarySmtpAddress') {
            _this6.SMTP = ChangeMessage;
            _this6.changespanshow.changeSMTPshow = true;
            _this6.getcomputermessagevalue();
          }
          _this6.$message({
            showClose: true,
      ...
      this.$prompt('请输入邮箱地址', {
        ...
        beforeClose: function beforeClose(action, instance, done) {
          if (action === 'confirm') {
            instance.confirmButtonLoading = true;
            instance.confirmButtonText = '执行中...';
            __WEBPACK_IMPORTED_MODULE_2_axios___default.a.get(_this7.serviceurl() + '/api/setDistributionGroupsmtp/?CountName="' + _this7.distinguishedName + '"&SmtpValue=' + instance.inputValue).then(function (response) {
              instance.confirmButtonLoading = false;
              if (response.data.isSuccess) {
                _this7.smtp.push(instance.inputValue);
                _this7.messagealertvalue('smtp添加成功', 'success');
              } else {
                if (response.data.message === '权限不足') {
                  _this7.messagealertvalue('权限不足', 'error');
                } else {
                  _this7.messagealertvalue('smtp添加失败', 'error');
                }
              }
              done();
            });
    ...
    addusertoexmailbox: function addusertoexmailbox() {
      ...
      if (this.groupType === 8 || this.groupType === -2147483640) {
        var disNameforurl = this.getQueryVariable('disName');
        this.$confirm('是否为群组' + disNameforurl + '开通邮箱?', '提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning',
          beforeClose: function beforeClose(action, instance, done) {
            if (action === 'confirm') {
              instance.confirmButtonLoading = true;
              instance.confirmButtonText = '执行中...';
              __WEBPACK_IMPORTED_MODULE_2_axios___default.a.get(_this8.serviceurl() + '/api/EnDistributionGroup/?CountName=' + _this8.distinguishedName).then(function (response) {
                instance.confirmButtonLoading = false;
                if (response.data.isSuccess) {
                  _this8.messagealertvalue('群组邮箱开通成功', 'success');
                  _this8.getcomputermessagevalue();
                } else {
                  if (response.data.message === '权限不足') {
                    _this8.messagealertvalue('权限不足', 'error');
                  } else {
                    _this8.messagealertvalue('群组邮箱开通失败', 'error');
                  }
                }
                done();
              });
        ...
        this.messagealertvalue('组类型不允许创建邮箱', 'error');
      }
    },
    ...
    getcomputermessagevalue: function getcomputermessagevalue() {
      ...
      __WEBPACK_IMPORTED_MODULE_2_axios___default.a.get(this.serviceurl() + '/api/GetGroupPreMessage/?CountName=' + disNameforurl).then(function (response) {
        if (response.data.isSuccess) {
          _this11.groupType = response.data.message.groupType;
          _this11.sAMAccountName = response.data.message.sAMAccountName;
          _this11.distinguishedName = response.data.message.distinguishedName;
          if (!response.data.message['proxyAddresses']) {
            ...
            if (response.data.message['proxyAddresses'].length === 0) {
              ...
              // this.msExchRequireAuthToSendTo = response.data.message.msExchRequireAuthToSendTo
              _this11.hassmtpvalue = true;
              __WEBPACK_IMPORTED_MODULE_2_axios___default.a.get(_this11.serviceurl() + '/api/GetMailGroup/?CountName=' + _this11.distinguishedName).then(function (response) {
                loading.close();
                if (response.data.isSuccess) {
                  ...
                  if (response.data.message === '权限不足') {
                    ...
                    _this11.messagealertvalue('群组邮箱信息获取失败', 'error');
                  }
                }
              });
            }
