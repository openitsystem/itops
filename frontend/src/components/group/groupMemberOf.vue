<template>
  ...
  <el-button
    ...
    type="danger"
    @click="removemoregroupfromuser">退出选中群组</el-button> -->
  <el-table height='500' id="selectgroupmemberoftable" @selection-change="selectionchangedvalue" :data="tableData3.filter(data => !search || data.date.toLowerCase().includes(search.toLowerCase()))" @row-dblclick="dataBackFillGoods" style="width: 100%" v-cloak>
...
export default{
  ...
  methods: {
    ...
    exportExcel () {
      ...
      let wb = XLSX.utils.table_to_book(document.querySelector('#selectgroupmemberoftable'))
