<template>
  ...
  <el-menu style="margin-bottom:10px" class="el-menu-demo" mode="horizontal">
    ...
    <el-submenu index="1">
      ...
      <el-submenu index="1-2">
        <template slot="title">邮件流设置</template>
        <el-menu-item index="1-2-1" @click="changeauthOrigdiagshow">邮件传递限制</el-menu-item>
        <el-menu-item index="1-2-2" @click="changespanshow.changemsExchRequireAuthToSendToshow = false, msExchRequireAuthToSendToChangevalue = msExchRequireAuthToSendTo">发件人身份验证</el-menu-item>
      </el-submenu>
    </el-submenu>
  ...
  <el-table :data="tableData2"  style="width: 100%" :show-header="false" v-cloak>
    <el-table-column width="180">
      <template slot-scope="scope">
        <el-tooltip class="item" effect="light" content="msExchRequireAuthToSendTo" placement="left-start"><span>&nbsp;发件人身份验证：</span></el-tooltip>
        <span style="font-size:130%;" v-if="changespanshow.changemsExchRequireAuthToSendToshow" :class="[classname.classSpanFloatRight,classname.classSpancursorpointer]" @click="changespanshow.changemsExchRequireAuthToSendToshow = false, msExchRequireAuthToSendToChangevalue = msExchRequireAuthToSendTo"><i class="el-icon-edit-outline"></i></span>
      </template>
    </el-table-column>
    <el-table-column>
      <template slot-scope="scope">
        <span v-if="changespanshow.changemsExchRequireAuthToSendToshow" v-text="msExchRequireAuthToSendTo"></span>
        <div v-else >
          <el-col :span="6">
            <span size="small" v-show="!msExchRequireAuthToSendToChangevalue" v-text="msExchRequireAuthToSendTochangemessagevalue.falsevmessagevalue"></span>
            <span size="small" v-show="msExchRequireAuthToSendToChangevalue" v-text="msExchRequireAuthToSendTochangemessagevalue.truevmessagevalue"></span>
            <el-switch
              v-model="msExchRequireAuthToSendToChangevalue">
            </el-switch>
          </el-col>
          <el-col :span="6" style="margin-left:auto;margin-right:auto;">
            &nbsp;<span :class="classname.classSpancursorpointer" v-loading.fullscreen.lock="vLoadingShow" @click="ChangeUserMessagefuction('msExchRequireAuthToSendTo' ,msExchRequireAuthToSendToChangevalue)"><i class="el-icon-upload2" style="font-size:150%;"></i></span>
            <span @click="changespanshow.changemsExchRequireAuthToSendToshow = true" :class="classname.classSpancursorpointer"><i class="el-icon-close" style="font-size:150%;"></i></span>
          </el-col>
        </div>
      </template>
    </el-table-column>
...
export default{
  ...
  methods: {
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            axios
              ...
              .get(this.serviceurl() + '/api/SetDistrGroup/?CountName=' + disNameforurl + '&parametername=PrimarySmtpAddress&parametervalue=' + smtpvalue)
              .then(response => {
                instance.confirmButtonLoading = false
                if (response.data.isSuccess) {
        ...
        .then(response => {
          if (response.data.isSuccess) {
            ...
            if (response.data.message === '权限不足') {
              ...
              this.messagealertvalue('群组邮箱信息获取失败', 'error')
            }
          }
        })
    },
    changemailboxvalue: function (Attributesname, ChangeMessage) {
      ...
      this.vLoadingShow = true
      axios
        .get(this.serviceurl() + '/api/SetDistrGroup/?CountName=' + this.distinguishedName + '&parametername=' + Attributesname + '&parametervalue=' + ChangeMessage)
        .then(response => {
          this.vLoadingShow = false
          if (response.data.isSuccess) {
      ...
      this.$prompt('请输入邮箱地址', {
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            instance.confirmButtonText = '执行中...'
            axios
              .get(this.serviceurl() + '/api/setDistributionGroupsmtp/?CountName="' + this.distinguishedName + '"&SmtpValue=' + instance.inputValue)
              .then(response => {
                instance.confirmButtonLoading = false
                if (response.data.isSuccess) {
        ...
        this.$confirm('是否为群组' + disNameforurl + '开通邮箱?', '提示', {
          ...
          beforeClose: (action, instance, done) => {
            if (action === 'confirm') {
              ...
              instance.confirmButtonText = '执行中...'
              axios
                .get(this.serviceurl() + '/api/EnDistributionGroup/?CountName=' + this.distinguishedName)
                .then(response => {
                  instance.confirmButtonLoading = false
                  if (response.data.isSuccess) {
                    this.messagealertvalue('群组邮箱开通成功', 'success')
                    this.getcomputermessagevalue()
                  } else {
                    if (response.data.message === '权限不足') {
                      this.messagealertvalue('权限不足', 'error')
                    } else {
                      this.messagealertvalue('群组邮箱开通失败', 'error')
                    }
                  }
                  done()
                })
      ...
      axios
        ...
        .then(response => {
          if (response.data.isSuccess) {
            ...
            this.distinguishedName = response.data.message.distinguishedName
            if (!response.data.message['proxyAddresses']) {
              ...
              if (response.data.message['proxyAddresses'].length === 0) {
                ...
                axios
                  .get(this.serviceurl() + '/api/GetMailGroup/?CountName=' + this.distinguishedName)
                  .then(response => {
                    loading.close()
                    if (response.data.isSuccess) {
